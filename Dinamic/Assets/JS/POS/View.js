...
class Checkout {
	...
	getCurrentPaymentValue = () =>
		parseFloat(checkout().paymentAmountInput().value) || 0;

	getCurrentPaymentData = ({ code, description }) => ({
		amount: checkout().paymentAmountInput().value,
		description: description,
		method: code,
		referencia: checkout().paymentReferenciaInput().value,
	});
...
class Main {
	...
	showticketImpresion = function (data) {
		...
		let ventanaImpresion = window.open("", "blank");
