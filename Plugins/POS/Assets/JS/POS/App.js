...
async function orderResumeAction({ code }) {
	...
	for (const element of elements) {
		if (
			element.dataset.code === Cart.doc.generadocumento &&
			element.dataset.serie === Cart.doc.codserie
		) {
...
async function showProductFamiliesAction({ code, madre }) {
	View.main().updateProductFamilyList(
		await Core.getProductFamilyChild(code, madre)
	);
}
